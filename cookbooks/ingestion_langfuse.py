"""AgentDiff — Ingest a Langfuse trace. Offline, no API keys.

Langfuse traces export as a parent trace with nested ``observations``.
AgentDiff auto-detects the ``observations`` key. To run:

    uv run cookbooks/ingestion_langfuse.py
"""
...
def main() -> None:
    with open(SAMPLE / "langfuse.json", encoding="utf-8") as fh:
        trace = parse_trace_data(json.load(fh), adapter_name="langfuse")
